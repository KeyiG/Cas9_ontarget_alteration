...
library(dplyr)
library(stringr)
Pol3_count_ontarget <- read.delim("/Users/keyigeng/Dropbox/illuminadata/ChIP_Cas9_ontargetpaper/Cas9_ontarget_pol3_counting.txt") %>%
...
Pol3_count_ontarget_target_p <- Pol3_count_ontarget_target_p %>%
  ...
  tibble::add_row(sample=c("HAP1_expected_del", "HepG2_expected_del"), Normalised_count=c(0,0)) %>%
  dplyr::mutate(cell=c("HepG2", "HAP1", "HAP1", "HepG2", "HAP1", "HepG2")) 
plot_pol3_occ = function(x) {
  ...
  ggplot(Pol3_count_ontarget_target_p %>% dplyr::filter(cell==x), aes(sample, Normalised_count, fill=sample)) +
    ...
    scale_fill_manual(values=c("#62B4EC", "white", "#FB6B01")) +
